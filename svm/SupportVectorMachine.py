...
class SVM():
    def loadDataSet(self,filename): #读取数据
        dataMat=[]
        labelMat=[]
        fr=open(filename)
    ...
    def selectJrand(self,i,m): #在0-m中随机选择一个不是i的整数
        j=i
        while (j==i):
            j=int(random.uniform(0,m))
        return j

    def clipAlpha(self,aj,H,L):  #保证a在L和H范围内（L <= a <= H）
        if aj>H:
            aj=H
        if L>aj:
            aj=L
    ...
    def kernelTrans(self,X, A, kTup): #核函数，输入参数,X:支持向量的特征树；A：某一行特征数据；kTup：('lin',k1)核函数的类型和参数
        m,n = shape(X)
        K = mat(zeros((m,1)))
        if kTup[0]=='lin': #线性函数
            K = X * A.T
    ...
    def calcEk(self,oS, k): #计算Ek（参考《统计学习方法》p127公式7.105）
    ...
    def updateEk(self,oS, k): #更新os数据
        Ek = self.calcEk(oS, k)
        oS.eCache[k] = [1,Ek]

    #首先检验ai是否满足KKT条件，如果不满足，随机选择aj进行优化，更新ai,aj,b值
    def innerL(self,i, oS): #输入参数i和所有参数数据
        Ei = self.calcEk(oS, i) #计算E值
        if ((oS.labelMat[i]*Ei < -oS.tol) and (oS.alphas[i] < oS.C)) or ((oS.labelMat[i]*Ei > oS.tol) and (oS.alphas[i] > 0)): #检验这行数据是否符合KKT条件 参考《统计学习方法》p128公式7.111-113
            j,Ej = self.selectJ(i, oS, Ei) #随机选取aj，并返回其E值
            ...
            oS.alphas[j] = self.clipAlpha(oS.alphas[j],H,L) #参考《统计学习方法》p127公式7.108
            self.updateEk(oS, j)
    ...
    #SMO函数，用于快速求解出alpha
    def smoP(self,dataMatIn, classLabels, C, toler, maxIter,kTup=('lin', 0)): #输入参数：数据特征，数据类别，参数C，阀值toler，最大迭代次数，核函数（默认线性核）
    ...
    def testRbf(self,data_train,data_test):
        dataArr,labelArr = self.loadDataSet(data_train) #读取训练数据
        b,alphas = self.smoP(dataArr, labelArr, 200, 0.0001, 10000, ('rbf', 1.3)) #通过SMO算法得到b和alpha
        datMat=mat(dataArr)
        ...
        errorCount = 0
        for i in range(m):
            kernelEval = self.kernelTrans(sVs,datMat[i,:],('rbf', 1.3)) #将支持向量转化为核函数
        ...
        dataArr_test,labelArr_test = self.loadDataSet(data_test) #读取测试数据
        errorCount_test = 0
        datMat_test=mat(dataArr_test)
        labelMat = mat(labelArr_test).transpose()
        m,n = shape(datMat_test)
        for i in range(m): #在测试数据上检验错误率
            kernelEval = self.kernelTrans(sVs,datMat_test[i,:],('rbf', 1.3))
...
#主程序
def main():
    filename_traindata='svmtraindata.txt'
    filename_testdata='svmtestdata.txt'
    SVM.testRbf(filename_traindata,filename_testdata)
